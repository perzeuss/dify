...
const AppDetailNav = ({ title, desc, icon, icon_background, navigation, extraInfo, iconType = 'app' }: IAppDetailNavProps) => {
  ...
  return (
    <div className="flex flex-col sm:w-56 w-16 overflow-y-auto bg-white dark:bg-neutral-900 border-r border-gray-200 shrink-0 mobile:h-screen">
      <div className="flex flex-shrink-0 p-4">
        <AppBasic mode={mode} iconType={iconType} icon={icon} icon_background={icon_background} name={title} type={desc} />
      </div>
      <nav className="flex-1 p-4 space-y-1 bg-white dark:bg-neutral-900">
