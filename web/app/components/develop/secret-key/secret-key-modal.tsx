    ...
    <Modal isShow={isShow} onClose={onClose} title={`${t('appApi.apiKeyModal.apiSecretKey')}`} className={`${s.customModal} px-8 flex flex-col`}>
                ...
                <div className='flex items-center text-sm font-normal text-gray-700 dark:text-gray-300 border-b border-solid h-9' key={api.id}>
                  ...
                  <div className='flex flex-grow px-3'>
                    <Tooltip
                      ...
                      className='z-10'
                    >
                      <div className={`flex items-center justify-center flex-shrink-0 w-6 h-6 mr-1 rounded-lg cursor-pointer hover:bg-gray-100 dark:bg-neutral-950 ${s.copyIcon} ${copyValue === api.token ? s.copied : ''}`} onClick={() => {
                        // setIsCopied(true)
                        copy(api.token)
      ...
      <div className='flex'>
        <Button type='default' className={`flex flex-shrink-0 mt-4 ${s.autoWidth}`} onClick={onCreate} disabled={ !currentWorkspace || currentWorkspace.role === 'normal'}>
          ...
          <div className='text-xs font-medium text-neutral-200'>{t('appApi.apiKeyModal.createNewSecretKey')}</div>
        </Button>
      </div>
