    ...
    <div className={cn('flex flex-wrap', isSpecialMode ? 'bg-gray-100 dark:bg-neutral-950 min-w-[200px] rounded-lg pb-1 pl-1' : '')}>
      {
        items.map((item, index) => (
          <div
            key={item}
            className={cn('flex items-center mr-1 mt-1 px-2 py-1 text-sm text-gray-700 dark:text-gray-300 border border-gray-200', isSpecialMode ? 'bg-white dark:bg-neutral-900 rounded-md' : 'rounded-lg')}>
            {item}
            {
              !disableRemove && (
                <X
